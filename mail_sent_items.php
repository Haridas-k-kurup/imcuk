...
<?php include_once('includes/profile_left.php'); ?>
    <div class="col-lg-9">
     ...
     <div id="parentHorizontalTab">
            ...
            <div class="resp-tabs-container hor_1">
              <?php   $msgQuery		=	"select msg.*, user.ud_name_title,user.ud_first_name from personal_messages as msg left join user_details as user on msg.msg_to = user.reg_id where msg.msg_from = '".$activeMem."' and msg.to_status = 1 and msg.msg_status = 1 order by msg.msg_id desc"; 
    ...
    <?php 
				...
				foreach($allMesg as $mesgs){
				 if($mesgs['msg_to'] == $activeMem){
						$from	=	"To: Me";
					}
					else if($mesgs['ud_name_title']){
						$from	=	"To: ".$mesgs['ud_name_title'].": ".$mesgs['ud_first_name'];
					}
					else{
						$from	=	"To: ".$mesgs['ud_first_name'];
					}
		 ?>
      <tr>
      ...
      <?php $i++; } ?>
                 ...
                 <div>
                   <table class="table table-hover mailBox" style=" border:2px solid #C0C0C0; border-bottom:3px solid #C0C0C0;">
                        <tbody>
                            <tr>
                            	<td colspan="5">Your tab is empty.</td>
                            </tr>
                        </tbody>
                    </table>
                 </div>
                 ...
                 <?php   $gMsgQuery		=	"select gmsg.*, grp.group_name from group_messages as gmsg left join group_info as grp on gmsg.group_id = grp.group_id left join group_members as mem on gmsg.group_id = mem.group_id where gmsg.gmsg_from = '".$activeMem."' and mem.reg_id = '".$activeMem ."' and gmsg.gmsg_status = 1 order by gmsg.gmsg_id desc"; 
