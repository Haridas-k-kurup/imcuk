<?php 
	...
	$ip			= $objCommon->get_ip();
	$topic		= $_GET['tid'];
	$selfPage	= SITE_ROOT."forum_post.php?tid=".$topic;
	$sql		= "select forum.*, user.*, sum(ulike.topic_like) as totlike, sum(ulike.topic_dislike) as disliks from forum_topics as forum left join user_details as user on forum.reg_id=user.reg_id left join forum_topic_like as ulike on forum.topic_id = ulike.topic_id where forum.topic_id 	=	'".$topic."' and forum.topic_staff_manage = 0 and forum.topic_status = 1 order by forum.topic_created_on desc";					
	$getToic	= $objTopic->getRowSql($sql);
	if(!$getToic['topic_id']){
		header('location:index.php');
	}
	$getLStatus	= $objTLike->getRow('topic_id = "'.$topic.'" and topic_like_ip = "'.$ip.'"');
	$timeNow	= date("Y-m-d H:i:s");
	$postDate	= $getToic['topic_created_on'];
	$diff 		= abs(strtotime($postDate) - strtotime($timeNow));
	$years 		= floor($diff / (365*60*60*24));
	$months 	= floor(($diff - $years * 365*60*60*24) / (30*60*60*24));
	$days 		= floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24));
	if($topic){
		$noview		= $getToic['topic_view'];
		$newView	= $noview+1;
		$objTopic->updateField(array("topic_view"=>$newView),"topic_id=".$topic);
	}
              ...
              <div class="col-lg-6 ">
               <a href="<?php echo $plabLink; ?>">
                <div style="border:5px solid #FFF; padding:5px;">
                  <div style="border:5px solid #000; padding:5px; color:#FFF; background-color:#400080; font-size:24px; font-weight:bold; text-align:center;">PLAB</div>
                </div></a>
              </div>
              <div class="col-lg-6 ">
              <a href="<?php echo $umleLink; ?>">
                <div style="border:5px solid #FFF; padding:5px;">
                  <div style="border:5px solid #000; padding:5px; color:#FFF; background-color:#6C006C; font-size:24px; font-weight:bold; text-align:center;">USMLE</div>
                </div>
                </a>
              </div>
              <div class="col-lg-12">
                <a href="http://drswamyplabcourses.co.uk/" target="_blank">   <div style="border:5px solid #FFF; padding:5px; margin-top:2%;">
                  <div style="border:5px solid #000; padding:5px; color:#FFF; background-color:#5C00B9; font-size:28px; font-weight:bold; text-align:center;">Dr Swamy PLAB Courses<br /> <span style="color:#FF0; font-size:28px; text-shadow: 2px 2px 2px #FF5B5B;">Email :- swamyplab@gmail.com <br />
...
Tel :-  0044 744  8070 710</span></div>
      ...
      <div class="row background2" style="margin-top:10px;" id="scroll_to_this">
        <div class="col-lg-12">
          ...
          <?php if (!empty($getToic['topic_notice'])) { ?>
          <div class="forum_ask_contant">
            <h3 style="color:#606; padding:10px 30px;">IMPORTANT NOTICE: DO NOT IGNORE</h3>
            <p style="color:#930; padding:10px 30px;">
            	<?php
					/*$getMessage			= $objMessage->getRow("message_code = 'forum-notice'");
					echo stripslashes($getMessage['message']);*/
					echo stripslashes($getToic['topic_notice']);
				 ?>
            </p>
          </div>
          <?php } ?>
          ...
          <div class="col-lg-9"><?php $dis_id = 0; ?>
            <div style="background-color:#e3eef4;" class="col-lg-12 forum_user_question ovr">
              ...
              <div id="ask_question_body<?php echo $topic."101";?>" style="color:#333" class="ask_question_body">
                <?php echo stripslashes($getToic['topic_desc']); ?> 
              </div>
              <div class="col-lg-12 ask_question_replay">
                ...
                <div class=" col-lg-6 ">
                  ...
                  <?php
					$cLen	=	960;
						if(strlen($getToic['topic_desc'])> $cLen){
					 ?>
         ...
         <!--FORUM DISCUSSION START-->
          <?php
				...
				//	$pg_param 						= 	"";
						 ...
						 $sql_pagination 			= 	"select * from forum_discussion as dis left join user_details as user on dis.reg_id=user.reg_id where dis.topic_id='".$topic."' and dis.dis_staff_manage = 0 and dis.dis_status = 1 order by dis.dis_created_on asc";
