<?php
	...
	$animateId			= $_GET['aniId'];
	if($_GET['saniId']){
		$subAniId		= $_GET['saniId'];
	}else{
		$subAniId		= 0;				
	}
	...
	$sb_cat				= mysql_real_escape_string($_GET['cat']);
	if($_GET['dept'] && $_GET['cat']){
		/*if($_GET['dept'] > 7){
			echo "<img src=\"images/under_construction_animated.gif\" style=\"margin: 0px 15%;\">"; exit;
		}*/
		 $mobileSql		= "select subCat.subcat_name, subPages.sub_id, subPages.sub_heading, subPages.sub_information  from manage_sub_pages as subPages left join manage_sub_category as subCat on subPages.subcat_id = subCat.subcat_id where subPages.sub_id = '".$sb_cat."' order by subCat.subcat_position desc";
...
Tel :-  0044 744  8070 710</span></div>
 ...
 <p style="float:right"><a href="<?php echo SITE_ROOT ?>contactus.php?c=<?php echo $sb_cat; ?>&t=submenu" class="classname">Click here to give Suggestions on this Topic</a></p>
  ...
  <?php 
       $allSubMenus		= $objSubMenu->getAll('sub_id ='.$menuHead['sub_id'],'position asc');
   ?>
  <ul class="nav nav-tabs1 tabmenu">
   <?php foreach ($allSubMenus as $keys=>$subMenus) { ?>
    <li class="<?php 
						if ($animateId) {
						if ($subMenus['sub_menu_id'] == $animateId ){ echo "active"; } } else {
							if ($keys == 0) {
								 echo "active";
							}
						} ?>" id="<?php echo $subMenus['sub_menu_id']; ?>"><a href="#home<?php echo trim($subMenus['sub_menu_id']); ?>"><?php echo ucfirst(strtolower($subMenus['sub_menu_name'])); ?></a></li>
    <?php } ?>
  </ul>
  ...
  <div class="tab-content">
   <?php foreach($allSubMenus as $key=>$subMenus) { 
			$subMenu		=	trim($subMenus['sub_menu_id']);
	?>
    <div id="home<?php echo trim($subMenus['sub_menu_id']); ?>" class="tab-pane fade  <?php 
						if($animateId){
						if($subMenus['sub_menu_id'] == $animateId){ echo "in active"; } } else {
							if ($key == 0) {
								 echo "in active";
							}
						} ?>">
       <?php echo stripcslashes($subMenus['sub_menu_details']); ?>
     <?php $allSubSubMenu	 = $objSubSub->getAll('sub_menu_id = '.$subMenus['sub_menu_id'],'position asc') ?>
  ...
  <ul class="nav nav-tabs1 tabmenu">
   <?php foreach ($allSubSubMenu as $keys=>$subSubMenu) { ?>
    <li class="<?php 
						if($animateId){ 
						if($subSubMenu['sub_sub_id'] == $animateId){ echo "active"; } else {
							if($keys == 0){
								 echo "active";
							}
						} } else {
						...
						}?>" id="subsub-head<?php echo $subSubMenu['sub_sub_id']; ?>"><a href="#subsub<?php echo $subSubMenu['sub_sub_id']; ?>"><?php echo $subSubMenu['sub_sub_menu']; ?></a></li>
    <?php } ?>
  </ul>
  ...
  <div class="tab-content">
     <?php foreach ($allSubSubMenu as $keys=>$subSubMenu) { ?>
    <div id="subsub<?php echo $subSubMenu['sub_sub_id']; ?>" class="tab-pane fade <?php 
						if ($animateId) {
						if ($subSubMenu['sub_sub_id'] == $animateId){ echo "in active"; } else {
							if ($keys == 0) {
								 echo "in active";
							}
						} } else {
     ...
     <?php echo stripcslashes($subSubMenu['sub_sub_menu_details']); ?>
    
     <?php 
			// for more details of this menu button area
			$subPluseId		=	$objSubPluse->getAll('sub_sub_id = "'.$subSubMenu['sub_sub_id'].'"','sub_sub_id');
			if(count($subPluseId) > 0){ ?>
                	<a href="<?php echo SITE_ROOT; ?>detail_view.php?dept=<?php echo $currentPage; ?>&subId=<?php echo $subSubMenu['sub_sub_id']; ?>&subMenu=<?php echo $subMenu; ?>" class="btn btn-primary pull-right" style="margin-top: -30px;">VIEW MORE</a>
		<?php 	} ?>
    </div>
    <?php } ?>
            ...
            <div style="border:8px double #000066; border-radius:10px;">
              ...
              <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="hidden" name="subMenuId" value="<?php echo $subMenu; ?>" class="subMenuId" />
                  </div>
                  <div class="form-group">
