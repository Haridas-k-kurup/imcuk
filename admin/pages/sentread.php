<?php 
	...
	if(isset($_GET['mail'])){
		...
		if(isset($del_id) && $del_id >0){
			...
			if($chkStatus['mail_from'] == $adminSession){
				$delFlag	=	1;
				$objMail->updateField(array("mail_from_del" =>$delFlag),"mail_id =".$del_id);
			}
			header('location:index.php?page=sentbox');
		}
		$megSql		=	"SELECT mail.mail_id, mail.mail_subject, mail.mail_body, mail.mail_created, ad.admin_username, ad.admin_type FROM admin_mailbox AS mail LEFT JOIN admin AS ad ON mail.mail_to = ad.admin_id WHERE mail.mail_id = '".$mailId."' AND mail.mail_from = '".$adminSession."' AND mail.mail_from_del = 0 ORDER BY mail.mail_id DESC";
