...
$nameTittle		=	mysql_real_escape_string($_POST['nameTittle']);
$regType		=	mysql_real_escape_string($_POST['regType']);
$proType		=	mysql_real_escape_string($_POST['proType']);
$stuType		=	mysql_real_escape_string($_POST['stuType']);
$country		=	mysql_real_escape_string($_POST['country']);
$state			=	mysql_real_escape_string($_POST['state']);
$city			=	mysql_real_escape_string($_POST['city']);
$ageRange		=	mysql_real_escape_string($_POST['age']);
...
/*-----------Pagination start----------------*/
	if($adminType == 1){
	 $loginSql 						= 	"select * from registration_details as reg left join user_details as user on reg.reg_id = user.reg_id left join user_professionals_details as prof on prof.reg_id = reg.reg_id where reg_status =1 "; 
			if($nameTittle){
			$loginSql 				.= 	"and user.ud_name_title like '%".$nameTittle."%'";
			}if($regType){
				$loginSql 			.= 	" and reg.reg_type = '".$regType."'";
			}if($country){
				$loginSql 			.= 	" and user.ud_country like '%".$country."%'";
			}if($state){
				$loginSql 			.= 	" and user.ud_state like '%".$state."%'";
			}if($city){
				$loginSql 			.= 	" and user.ud_city like '%".$city."%'";
			}if($ageRange){
				$loginSql 			.= 	" and user.ud_age = '".$ageRange."'";
			}if($stuType){
				$loginSql 			.= 	" and prof.up_student_course like '%".$stuType."%'";
			}if($proType){
				$loginSql 			.= 	" and prof.up_profession_type like '%".$proType."%'";
			}if($startDate && $endDate){
				$loginSql 			.= 	" and reg.reg_createdon between '".$startDate."' AND '".$endDate."'";
			}
	?>
	<?php 
		 $loginSql					.= " order by reg.reg_id desc";
	}else{
		$loginSql 					= 	"select * from registration_details as reg left join user_details as user on reg.reg_id = user.reg_id left join user_professionals_details as prof on prof.reg_id = reg.reg_id where reg.reg_staff_manage = 0 ";
		if($nameTittle){
			$loginSql 				.= 	" user.ud_name_title like '%".$nameTittle."%'";
			}if($regType){
				$loginSql 			.= 	" and user.ud_name_title like '%".$regType."%'";
			}if($country){
				$loginSql 			.= 	" and user.ud_country like '%".$country."%'";
			}if($state){
				$loginSql 			.= 	" and user.ud_state like '%".$state."%'";
