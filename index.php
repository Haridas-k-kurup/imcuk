<?php
	...
	$secondQuery	= "select pages.* from manage_pages as pages left join manage_page_connection as con on pages.mp_id = con.mp_id where page_id = '".$currentPage."' and pages.mp_staff_manage = 0 and pages.mp_status=1 and con.cat_id = 10 and con.mcp_status = 1 order by pages.mp_createdon desc";
	$secondInfo 	= $objMgPage->listQuery($secondQuery);
...
Tel :-  0044 744  8070 710</span></div>
      ...
      <div class="row background2" style="margin-top:10px;">
        <div class="col-lg-12 padding-0">
          ...
          <p style="float:right"><a href="<?php echo SITE_ROOT ?>contactus.php?c=<?php echo $sb_cat; ?>&t=homeinfo" class="classname">Click here to give Suggestions on this Topic</a></p>
        </div>
      </div>
      <?php } ?>
       <?php if (count($mobileMenu)>0) { ?>
      <div class="row background2" style="margin-top:10px;">
       <?php 		$i				=  1;
		   foreach ($mobileMenu as $mobile) { 
					$subMenuId		=  $mobile['sub_id'];
					$imgSrc			=  stripslashes($mobile['sub_information']);
					preg_match('/<img.+src=[\'"](?P<src>.+)[\'"].*>/i',$imgSrc,$mobImage);
		?>
        <div class="col-sm-6 col-lg-4">
          <div class="column_contant_division ovr" style="width:100%">
            <div class="column_contant_details ovr">
              <div class="column_contant_head">
                <center>
                  <a href="<?php SITE_ROOT; ?>multimedia.php?dept=<?php echo $currentPage; ?>&cat=<?php echo $mobile['sub_id'];  ?>">
                  <h1 class="imcHeading1"><?php echo $objCommon->html2text($mobile['subcat_name']); ?></h1>
                  </a>
                </center>
              </div>
               <?php if (stripslashes($mobImage['src'])) { ?>
              <div class="column_contant_img ovr"> <img width="100%" height="120" src="<?php echo stripslashes($mobImage['src']); ?>" style=""> </div>
              <?php } ?>
              <div style="overflow-y:scroll;" class="img_and_details">
                <div class="vertical-menu">
                <?php $allSubMenus		=	$objSubMenu->getAll('sub_id = "'.$subMenuId.'"', 'position asc'); ?>
                  <ul>
                  <?php foreach ($allSubMenus as $allsub) { ?>
                  <?php $allSubSubMenu	=	$objSubSub->getAll('sub_menu_id = '.$allsub['sub_menu_id'],'position asc') ?>
                    <li class="color-orange"> <a href="<?php echo SITE_ROOT ?>multimedia.php?dept=<?php echo $currentPage; ?>&cat=<?php echo $mobile['sub_id'];  ?>&aniId=<?php echo $allsub['sub_menu_id']; ?>"><?php echo strip_tags(ucfirst(strtolower($allsub['sub_menu_name']))); ?>
                    <?php if (count($allSubSubMenu) > 0) :  ?><span> <i class="fa fa-angle-right"></i> </span> <?php endif; ?> </a>
                      <ul class="drop-down">
                        <!-- dropdown -->
                        <?php foreach ($allSubSubMenu as $keys=>$subSubMenu) { ?>
                        <li><a href="<?php echo SITE_ROOT ?>multimedia.php?dept=<?php echo $currentPage; ?>&cat=<?php echo $mobile['sub_id'];  ?>&aniId=<?php echo $allsub['sub_menu_id']; ?>&saniId=<?php echo $subSubMenu['sub_sub_id']; ?>"><?php echo ucfirst(strtolower($subSubMenu['sub_sub_menu'])); ?></a></li>
                         <?php } ?>
                      </ul>
                    </li>
                    <?php } ?>
                    <li class="color-orange"><a href="javascript:;">...</a></li>
                  </ul>
                </div>
              </div>
            </div>
	...
	<?php	}
      ...
      <?php } ?>
   <?php   if ($secondInfo [0]['mp_heading']) {  
	   ?>
      <div class="row background2" style="margin-top:10px;">
        <div class="col-lg-12 padding-0">
          <div class="imc-pagemenu">
            <center>
              <?php echo stripslashes($secondInfo[0]['mp_heading']); ?>
            </center>
          </div>
          <div class="imc_heading1">
            <center>
          ...
          <?php echo stripslashes($secondInfo[0]['mp_desc']); ?>
          <p style="float:right"><a href="javascript:;" class="classname">Click here to give Suggestions on this Topic</a></p>
        </div>
      </div>
      <?php } ?>
       <?php 
			/*-----------Topic start----------------*/
				...
				//	$num_page_links_per_page 		= 7;
					...
					$sql_pagination 				= "select user.ud_name_title,user.ud_first_name,user.ud_pofile_pic,topic.topic_id, topic.page_id,  topic.topic,topic.topic_desc,topic.topic_view,dis.dis_created_on,topic.topic_created_on,count(dis.dis_id) as countrely from forum_topics as topic left join user_details as user on topic.reg_id = user.reg_id left join forum_discussion as dis on topic.topic_id=dis.topic_id ";
					
					if ($currentPage !=1) {
						$sql_pagination 			.= " where topic.page_id='".$currentPage."' and topic.topic_staff_manage = 0 and topic.topic_status = 1";
					} else {
						$sql_pagination 			.= " where topic.topic_staff_manage = 0 and topic.topic_status = 1";
					}
      ...
      <div class="row"  style="margin-top:10px;">
        ...
        <div class="row" >
          ...
          <?php 
			...
			$newImg			= str_replace("http://localhost:81/imc-new/",SITE_ROOT,$dImage[1]);
			 ?>
        <div class="row" >
          <div class="col-lg-12"  style="border:2px solid #999; margin-top:5px;">
            <div class="row"  style="border:1px solid #999">
            ...
            <div class="row">
              ...
              <div class="col-lg-9">
                <p><?php echo substr(strip_tags($all['topic_desc']),0,80); ?><a style="font-weight:bold; color:#910000" href="<?php echo SITE_ROOT; ?>forum_post.php?tid=<?php echo $all['topic_id']; ?>&dept=<?php echo $all['page_id']; ?>">&nbsp;&nbsp; Read more</a></p>
              </div>
            </div>
          </div>
          </div>
        ...
        <div class="row" >
          <div class="col-lg-12"  style="border:2px solid #999; margin-top:5px;">
           ...
           <?php 
				...
				if ($currentPage !=1) {
				$CommentWise    .= " where topic.page_id='".$currentPage."' and topic.topic_staff_manage = 0 and topic.topic_status = 1";
				} else {
				$CommentWise 	.= " where topic.topic_staff_manage = 0 and topic.topic_status = 1 ";
				}
              ...
              <form action="action.php?act=create_new_thread" method="post" enctype="multipart/form-data">
                  <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" name="topic" placeholder="Enter topic ..." class="form-control" title="Forum Topic" required="required">
                        <input type="hidden" name="page_id" value="<?php echo $currentPage; ?>">
                      </div>
                      <div class="form-group">
                        <label>Forum Description</label>
                         <textarea id="editor1" name="topic_desc"  class="ckeditor admin_add_content_textarea" style="width:100%" placeholder="Write here..." ></textarea>
                      </div>
                  </div>
