<?php 
	...
	if(isset($_GET['g'])){
		$groupId	=	$objCommon->esc($_GET['g']);
		$groupName	=	$objGroup->getRow('group_id = "'.$groupId.'"');	
		$validGroup	=	$objMember->getRow('group_id = "'.$groupId.'" and reg_id = "'.$activeMem.'"');// privent from show others groups
		if(isset($_GET['search'])){
		$searchName	=	$objCommon->esc(trim($_GET['search']));
		$memQuery	=	"select grp.group_name, mem.group_m_id, mem.reg_id, mem.group_m_status, user.ud_name_title, user.ud_first_name, user.ud_country, user.ud_pofile_pic from group_members as mem left join group_info as grp on mem.group_id = grp.group_id left join registration_details as reg on mem.reg_id = reg.reg_id left join user_details as user on reg.reg_id = user.reg_id where user.ud_first_name like '%".$searchName."%' and mem.group_id = '".$groupId."' order by user.ud_first_name";
		}else{
		$memQuery	=	"select grp.group_name, mem.group_m_id, mem.reg_id, mem.group_m_status, user.ud_name_title, user.ud_first_name, user.ud_country, user.ud_pofile_pic from group_members as mem left join group_info as grp on mem.group_id = grp.group_id left join registration_details as reg on mem.reg_id = reg.reg_id left join user_details as user on reg.reg_id = user.reg_id
		where mem.group_id = '".$groupId."' order by user.ud_first_name";
		}
		$memAll		=	$objMember->listQuery($memQuery);
		}else{
 ...
 <!--TOP SLIDER START-->
  <div class="row" style="padding:10px 0px;">
    ...
    <div class="col-lg-9">
     <div class="row">
     <div class="col-lg-12">
       <?php   if ($validGroup['group_id'] == $groupName['group_id']) { ?>
     <div class="col-lg-9">
     
     <span style="font-size:24px; color:#333;"><?php echo $objCommon->esc($groupName['group_name']); ?></span> <a class="update-btn fr" href="<?php echo SITE_ROOT; ?>update_information.php">Update Info</a>
        <hr style="padding:0px; margin:0px 0px 10px 0px ; color:#000; border:1px solid #666;"/>
        <form method="get" id="mem-search">
        <input type="hidden" name="g" value="<?php echo $groupId; ?>" />
       <label>
<i class="fa fa-user userbtn" title="Find a Member"></i>
<input class="userbtninput" type="text" value="<?php echo ($_GET['search']) ? $searchName : '' ?>" placeholder="Find a Member" name="search" kl_virtual_keyboard_secure_input="on">
<a id="search-bt" href="javascript:;">
<i class="fa fa-search userbtn"></i>
</a>

</label>   
 <a class="add-member-btn" href="<?php echo SITE_ROOT; ?>find_members.php"><i class="fa fa-user-plus"></i> Add Members</a>  
</form>   
 <div class="row">
 <?php						
	$i	= 1;
    foreach ($memAll as $member) { ?>
 
 <div class="col-lg-4">
 
 <div class="boxshadowa1">
 <div class="row">
 <div class="col-lg-4">
  <?php if($member['ud_pofile_pic']){
							...
							?>
                            <img src="<?php echo SITE_ROOT.$profPic; ?>" class="addmember_img" style="height:75px;" >
                            <?php } else{ ?>
                            <img src="<?php echo SITE_ROOT; ?>images/profile.jpg" class="addmember_img" style="height:75px;" >
                            <?php }  }else{ ?>
                            <img src="<?php echo SITE_ROOT; ?>images/profile.jpg" class="addmember_img"  style="height:75px;" >
                            <?php } ?>
 ...
 <div class="col-lg-8">
 <p class="addmember_text" style=" <?php echo ($member['group_m_status'] == 0) ? "color:#F60" : "" ?> "><?php echo stripslashes($member['ud_first_name']); ?></p>
 <div style="font-size:14px;" style=" <?php echo ($member['group_m_status'] == 0) ? "color:#F60" : "" ?> "><?php echo stripslashes($member['ud_country']); ?> 
 <?php if($groupName['reg_id'] == $activeMem){ ?>
 <span class="pull-right drop1down"><i class="fa fa-cog drop1btn"></i>
        <div class="drop1down-content">
    <a href="javascript:;" onclick="return manageMember(1,<?php echo $member['group_m_id']; ?>);"><?php  if ($member['group_m_status'] == 0) {  ?> <i class="fa fa-unlock"></i>&nbsp;&nbsp; Unblock
                                                      <?php } else { ?>
                                                       <i class="fa fa-ban"></i>&nbsp;&nbsp; Block
                                                      <?php } ?></a>
    <a href="javascript:;" onclick="return manageMember(2,<?php echo $member['group_m_id']; ?>);"><i class="fa fa-times"></i> Remove</a>
  </div>
        </span>
        <?php } ?>
        </div>
  ...
  <?php $i++; }  ?>
     ...
     <?php } else {
				header('location:profile_group_details.php');
			} ?>
     <div class="col-lg-3">
...
<div class="modal fade" id="myModal1" role="dialog">
    ...
    <div style="border:8px double #000066; border-radius:10px;">
        ...
        <div class="col-lg-8"><input type="password" style="width:100%" required placeholder="   Enter New Password" class="inputstyle1" name="curpass" kl_virtual_keyboard_secure_input="on"></div>
        </div>
        <div class="row">
        <div class="col-lg-4 labletext1">Re-Type Password</div>
        <div class="col-lg-8"><input type="password" style="width:100%" required placeholder="  Re-Enter New Password" class="inputstyle1" name="curpass" kl_virtual_keyboard_secure_input="on"></div>
        </div>
        </div>
...
<script type="text/javascript" language="javascript">
   ...
   <script type="text/javascript" language="javascript">
 	function manageMember(act,m){ // FUNCTION FOR MANAGE GROUP MEMBERS
		if(act&&m){
		var dataString	=	"act="+act+"&m="+m;
		$.ajax({
				type:"POST",
				url:"ajax/manage_group_members.php",
				data:dataString,
				cache:false,
				success:function(data){
					location.reload();
