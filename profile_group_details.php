<?php 
	...
	if($sessionval	==	false){
		...
		$memQuery	=	"select grp.*, mem.group_m_id from group_members as mem left join group_info as grp on mem.group_id = grp.group_id where mem.reg_id = '".$activeMem."' and grp.reg_id != '".$activeMem."' order by grp.group_id desc";
 ...
 <!--TOP SLIDER START-->
  <div class="row" style="padding:10px 0px;">
    ...
    <div class="col-lg-9">
     ...
     <div class="col-lg-12">
     <div class="col-lg-9">
     <span style="font-size:24px; color:#333;">Groups( <?php echo $groupCount; ?> )</span> <a class="update-btn fr" href="<?php echo SITE_ROOT ?>update_information.php">Update Info</a>
        <hr style="padding:0px; margin:0px 0px 10px 0px ; color:#000; border:1px solid #666;"/>
        <p style="font-weight:bold;">Groups You Manage</p>
        <?php
								if(count($groupAll) >0){
								 foreach($groupAll as $group){ ?>
        <div class="group_msg1"><?php echo $objCommon->esc($group['group_name']); ?><span class="pull-right dropdown"><i class="fa fa-cog dropbtn"></i>
        <div class="dropdown-content">
    <a href="<?php echo SITE_ROOT; ?>profile_group_members.php?g=<?php echo $group['group_id']; ?>"><i class="fa fa-search"></i> View Members</a>
    <a href="javascript:;" onclick="return deleteGroup(<?php echo $group['group_id']; ?>);"><i class="fa fa-trash"></i> Delete Group</a>
    <a href="<?php echo SITE_ROOT ?>find_members.php"><i class="fa fa-plus"></i> Add Member</a>
  </div>
        </span></div>        
         <?php } } else{ ?>        
                 <div class="group_msg1">No group created</div> 
        <?php	}?> 
        
        
        <p style="font-weight:bold; margin-top:10px;">Group you are Created</p>
        <?php
									if(count($memAll)){ 
								 foreach($memAll as $member){ ?>
        <div class="group_msg1"><?php echo $objCommon->esc($member['group_name']); ?><span class="pull-right dropdown"><i class="fa fa-cog dropbtn"></i>
        <div class="dropdown-content">
    <a href="<?php echo SITE_ROOT; ?>profile_group_members.php?g=<?php echo $member['group_id']; ?>"><i class="fa fa-search"></i> View Members</a>
    <a href="javascript:;" onclick="return leaveGroup(<?php echo $member['group_m_id']; ?>)"><i class="fa fa-trash"></i> Delete Group</a>
    <a href="find_members.html"><i class="fa fa-plus"></i> Add Member</a>
  </div>
        </span></div>                
          <?php } } else { ?>
           <div class="group_msg1">You are not there in any group</div> 
          <?php } ?>       
...
<script type="text/javascript" language="javascript">
 ...
 function leaveGroup(g){
		if(g){ 
			if(confirm("Are you sure ! Do you want to leave this group")){
			var dataString	=	"g="+g;
			$.ajax({
				type:"POST",
